alert("Bienvenido al lugar preferido de las mascotas.");
alert("Todo pago es realizado mediante tarjeta.");
alert("Forma parte de la familia PetShop, registrate y obtén promociones únicas.");
...
function Registrarse(){
  ...
  númerodeteléfono = window.prompt("Ingrese su número de teléfono:");
   if(isNaN(númerodeteléfono)){
    ...
    númerodeteléfono = window.prompt("Ingrese su número de teléfono, porfavor.");
  ...
  correo = window.prompt("Ingrese su correo electrónico");
     ...
     window.alert("Tu registro ha sido completado, " + nombrecompleto + ". Ya esta todo listo para que procedas a comprarle lo mejor a tu mascota");
...
function Agregar(){
  var addproductos = window.alert("Te mostraremos nuestra lista de productos para que los puedas acumular en tu carrito de compras.");
  var textoproductos ="";
  for(var i=0; i<productos.length; i++){
  textoproductos+=((i+1)+": "+productos[i][0]+"\n");
}
var productoseleccionado= window.prompt("Escriba el número que le corresponde al producto." + " \n" +textoproductos);
...
function FINALIZAR(){
 ...
 tarjeta = window.prompt("Esta bien, " + nombrecompleto + ". Ingrese el código de su tarjeta");
  if(isNaN(tarjeta)){
    window.alert("Debe de ingresar números.");
    tarjeta = undefined;
